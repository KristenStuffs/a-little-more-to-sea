package alittlemoretosea.mod.entity.rod.obsidianhook;
...
public class EntityObsidianHook extends EntityFishHook
{
    private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager.<Integer>createKey(EntityObsidianHook.class, DataSerializers.VARINT);
    ...
    private EntityObsidianHook.State currentState = EntityObsidianHook.State.FLYING;
    private int luck;
    private int lureSpeed;

    @SideOnly(Side.CLIENT)
    public EntityObsidianHook(World worldIn, EntityPlayer p_i47290_2_, double x, double y, double z)
    {
    ...
    public EntityObsidianHook(World worldIn, EntityPlayer fishingPlayer)
    {
    ...
    public void onUpdate()
    {
        ...
        else if (this.world.isRemote || !this.shouldStopFishing())
        {
            ...
            if (this.currentState == EntityObsidianHook.State.FLYING)
            {
                if (this.caughtEntity != null)
                {
                    ...
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityObsidianHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F)
                {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityObsidianHook.State.BOBBING;
                    return;
                }
            ...
            {
                if (this.currentState == EntityObsidianHook.State.HOOKED_IN_ENTITY)
                {
                    if (this.caughtEntity != null)
                    {
                        if (this.caughtEntity.isDead)
                        {
                            ...
                            this.currentState = EntityObsidianHook.State.FLYING;
                        }
                        else
                        {
                ...
                if (this.currentState == EntityObsidianHook.State.BOBBING)
                {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
