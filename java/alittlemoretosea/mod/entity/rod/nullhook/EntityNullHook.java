package alittlemoretosea.mod.entity.rod.nullhook;
...
public class EntityNullHook extends EntityFishHook
{
    public static final ResourceLocation LOOT = new ResourceLocation(Reference.MODID, "");

	
    private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager.<Integer>createKey(EntityNullHook.class, DataSerializers.VARINT);
    ...
    private EntityNullHook.State currentState = EntityNullHook.State.FLYING;
    private int luck;
    private int lureSpeed;

    @SideOnly(Side.CLIENT)
    public EntityNullHook(World worldIn, EntityPlayer p_i47290_2_, double x, double y, double z)
    {
    ...
    public EntityNullHook(World worldIn, EntityPlayer fishingPlayer)
    {
    ...
    public void onUpdate()
    {
        ...
        else if (this.world.isRemote || !this.shouldStopFishing())
        {
            ...
            if (this.currentState == EntityNullHook.State.FLYING)
            {
                if (this.caughtEntity != null)
                {
                    ...
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityNullHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F)
                {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityNullHook.State.BOBBING;
                    return;
                }
            ...
            {
                if (this.currentState == EntityNullHook.State.HOOKED_IN_ENTITY)
                {
                    if (this.caughtEntity != null)
                    {
                        if (this.caughtEntity.isDead)
                        {
                            ...
                            this.currentState = EntityNullHook.State.FLYING;
                        }
                        else
                        {
                ...
                if (this.currentState == EntityNullHook.State.BOBBING)
                {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
    ...
    private boolean shouldStopFishing()
    {
        ...
        boolean flag = itemstack.getItem() instanceof alittlemoretosea.mod.objects.items.tools.rods.ItemNullRod;
        boolean flag1 = itemstack1.getItem() instanceof alittlemoretosea.mod.objects.items.tools.rods.ItemNullRod;
    ...
    public int handleHookRetraction()
    {
        if (!this.world.isRemote && this.angler != null)
        {
            ...
            else if (this.ticksCatchable > 0)
            {
                ...
                List<ItemStack> result = this.world.getLootTableManager().getLootTableFromLocation(LOOT).generateLootForPools(this.rand, lootcontext$builder.build());
                ...
                for (ItemStack itemstack : result)
                {
                    ...
                    this.angler.world.spawnEntity(new EntityXPOrb(this.angler.world, this.angler.posX, this.angler.posY * 0.0D, this.angler.posZ + 0.5D, this.rand.nextInt(6) + 1));
